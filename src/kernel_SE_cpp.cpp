...
Rcpp::List grad_SE_cpp(arma::colvec y, arma::mat X, arma::colvec z,arma::colvec w, arma::mat Kmat, arma::mat Km, arma::mat Ka, arma::mat invKmatn, Rcpp::List parameters) {
  ...
  //Rcpp::Rcout << ybar.size() <<std::endl;  
  //Rcpp::Rcout << alpha.size() <<std::endl;  
  //Rcpp::Rcout << tmpK.size() <<std::endl;    
  ...
  gradients["Lm"] = as<std::vector<double> >(tmp["m"]); //(tmp);//["m"]); //returns vector not matrix 
  gradients["La"] = as<std::vector<double> >(tmp["a"]); //(tmp);//["a"]); // as<std::vector<double> >(tmp["a"])
