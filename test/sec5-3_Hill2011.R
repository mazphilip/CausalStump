...
## Function for random surface generation ####
...
run <- function(i){
  ...
  rm(y.hat.all,tau.hat.all,ate.ci.train,ate.ci.test,tau.ci.all)
  ## GP #####################################################################
  #myweights = pscore.train*Z.train/mean(Z.train)+(1-pscore.train)*(1-Z.train)/(1-mean(Z.train))
  system.time(GP_fit <- CausalStump::CausalStump(y = Y.train,X = data.frame(X.train), z = Z.train,#w = myweights,
                                       maxiter=3000,tol=1e-1,learning_rate = 0.01,prior=FALSE,myoptim = "Nadam"))
  ...
  ## GP with PS #####################################################################
  GP_PS_fit = CausalStump::CausalStump(y = Y.train,X = data.frame(X.train), z = Z.train,pscore=pscore.train,
  ...
  ## TP2 with PS #####################################################################
  TP2_PS_fit = CausalStump::CausalStump(y = Y.train,X = data.frame(X.train), z = Z.train,pscore=pscore.train,
  ...
  rm(y.hat.all,tau.hat.all,ate.ci.train,ate.ci.test,tau.ci.all)
  ## TP2000 with PS #####################################################################
  TP2000_PS_fit = CausalStump::CausalStump(y = Y.train,X = data.frame(X.train), z = Z.train,pscore=pscore.train,
...
save.image(file = paste("H11_",max_iter,".RData",sep=""))
